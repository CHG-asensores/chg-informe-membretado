...
def generar_docx_con_observaciones(data, observaciones):
    doc = DocxDocument()
    ...
    for etiq, val in [("N de Orden", "#"+str(meta.get("numero_orden","-"))),("Estado",meta.get("estado","-")),("Ubicacion",meta.get("ubicacion","-")),("Activo",meta.get("activo","-")),("Asignados",", ".join(meta.get("asignados",[]))or"-"),("Fecha",meta.get("fecha")or datetime.date.today().strftime("%d/%m/%Y"))]:
        fila = tabla.add_row()
