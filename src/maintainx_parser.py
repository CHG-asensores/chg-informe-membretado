...
def parse_pdf(pdf_bytes, membrete_bytes):
    ...
    if len(doc) > 0:
        ...
        for entry in lines:
            text = entry["text"]
            if text in {"CHG Ascensores", "CHG", "ASCENSORES", "Campos completados", "INFORME DE MANTENIMIENTO PREVENTIVO"}:
                continue
    ...
    for page in doc:
        ...
        # A. Tapar cabecera original (Elimina logo MaintainX y metadatos superiores)
        page.draw_rect(fitz.Rect(0, 0, w, 75), color=(1,1,1), fill=(1,1,1))

        # B. Tapar pie de página original (Elimina "Generado para CHG..." inferior)
        page.draw_rect(fitz.Rect(0, h - 60, w, h), color=(1,1,1), fill=(1,1,1))

        # C. Insertar el membrete transparente por encima (overlay=True)
        # Esto colocará tu diseño sin tapar el centro de la hoja
        page.insert_image(page.rect, stream=membrete_bytes, overlay=True)

        # D. Insertar el texto de pie de página personalizado
        page.insert_text(
            (40, h - 18),
    ...
    # Retornamos estructura compatible con app.py
    return {
        "meta": meta,
